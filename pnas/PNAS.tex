...
\affil[a]{Department of Mechanical Engineering, National Central University, Taoyuan, 32001 Taiwan and\\
Department of Computational Methods in Systems and Control Theory, Max Planck Institute for Dynamics of Complex Technical Systems, 39106 Magdeburg, Germany\\}
...
\correspondingauthor{\textsuperscript{1}To whom correspondence should be addressed. E-mail: lglin@g.ncu.edu.tw or ywliang@cn.nctu.edu.tw}
...
\hspace{-0.58cm}\textbf{Initial Conditions:} $\bar \calL=\bar \calX=\bar \calI=\tilde \calI^*=\emptyset$
\Ensure The optimal value $\tilde l^*$ and the (resp., a subset of) corresponding optimum (resp., optima) $\bfx^{\tilde *}$
\newpage\State\label{Alg_A_V_2} compute the SVD of $A$ till $\calR(V_2)=\calN(A)$ is obtained
\vspace{0.1cm}
...
\vspace{0.2cm}\begin{minipage}{0.455\textwidth}
...
x_star_particular=...|\label{Ex_MATLAB_Implementation_Optimum_1}|
  A_dagger*b-V_2*pinv(V_2'*P*V_2)...|\label{Ex_MATLAB_Implementation_Optimum_2}|
  *(V_2'*q+V_2'*P*A_dagger*b);|\label{Ex_MATLAB_Implementation_Optimum_3}|
l_star_equilibrium=A_dagger*b+s-q'*V_2...|\label{Ex_MATLAB_Implementation_Optimal_Value_1}|
  *(eye(n)-V_2*pinv(V_2'*P*V_2)*V_2'*P)...|\label{Ex_MATLAB_Implementation_Optimal_Value_2}|
  *(b'*A_dagger'*P/2+q')...|\label{Ex_MATLAB_Implementation_Optimal_Value_3}|
  *pinv(V_2'*P*V_2)*V_2'*q/2;     (i)|\label{Ex_MATLAB_Implementation_Optimal_Value_4}|
\end{lstlisting}
\end{minipage}
...
\acknow{The authors thank Julian Mellor and Scott Read for their assistance in the native use of this language. This work was supported by the Max Planck Society, Germany; and the Ministry of Science and Technology, Taiwan.}
